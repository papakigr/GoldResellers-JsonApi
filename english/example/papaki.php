...
function papaki_registerdomain($params) {
	...
	$json = new Services_JSON();
	$jsonarray=array("request"=>array("do"=>'domainregister',"username"=>$username,"password"=>$password,"apiKey"=>$apikey,"domainname"=>$params["domain"],"description"=>$description,"ip1"=>' ',"ip2"=>' ',"ip3"=>' ',"ip4"=>' ',"ns1"=>encodetolatin($params["ns1"]),"ns2"=>encodetolatin($params["ns2"]),"ns3"=>encodetolatin($params["ns3"]),"ns4"=>encodetolatin($params["ns4"]),"owner_fullname"=>encodetolatin($params["companyname"]),"owner_firstname"=>encodetolatin($params["firstname"]),"owner_lastname"=>encodetolatin($params["lastname"]),"owner_email"=>encodetolatin($params["email"]),"owner_address"=>encodetolatin($params["address1"]),"owner_state"=>encodetolatin($params["state"]),"owner_city"=>encodetolatin($params["city"]),"owner_postcode"=>encodetolatin($params["postcode"]),"owner_country"=>encodetolatin($params["country"]),"owner_phone"=>encodetolatin($params["phonenumber"]),"owner_fax"=>$params["fax"],"owner_litepsd"=>' ',"owner_title"=>' ',"regperiod"=>$params["regperiod"],"idprotect"=>$idprotection,"customer_language"=>"gr"));
	$Xpost = $json->encode($jsonarray);
	$Xpost=latintogreek($Xpost);
...
function papaki_RenewDomain($params) {
	...
	# If error, return the error message in the value below
			...
			$json = new Services_JSON();
	$jsonarray=array("request"=>array("do"=>'domainupdate',"username"=>$username,"password"=>$password,"apiKey"=>$apikey,"domainname"=>$sld.".".$tld,"customer_language"=>'gr',"regperiod"=>$regperiod,"idprotect"=>$idprotection));
	$Xpost = $json->encode($jsonarray);
	$Xpost=latintogreek($Xpost);
